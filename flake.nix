  ...
  outputs = {
    ...
    flake-utils.lib.eachDefaultSystem (
      system: let
        ...
        staticPkgs =
          if systemPkgs.stdenv.targetPlatform.isLinux
          then systemPkgs.pkgsMusl
          else systemPkgs;
        ...
        echidnaRedistributable = pkgs:
          with pkgs; let
            ...
            in
              ...
              then
                pkgs.runCommand "echidna-stripNixRefs" {} ''
                  mkdir -p $out/bin
            ...
            stripDylib = drv:
              ...
              '';
          in
            echidnaRedistributable;
      in rec {
        packages.echidna = echidna systemPkgs;
        packages.default = echidna systemPkgs;

        packages.echidna-redistributable = echidnaRedistributable staticPkgs;
        packages.echidna-redistributable-aarch64-linux = echidnaRedistributable systemPkgs.pkgsCross.aarch64-multiplatform-musl;
        packages.echidna-redistributable-aarch64-darwin = echidnaRedistributable systemPkgs.pkgsCross.aarch64-darwin;

        devShell = with systemPkgs;
          haskellPackages.shellFor {
            packages = _: [echidna systemPkgs];
            shellHook = "hpack";
            buildInputs = [
