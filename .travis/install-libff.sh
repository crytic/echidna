...
if [ "$(uname)" == "Darwin" ]; then
  ...
  ARGS="$ARGS -DOPENSSL_INCLUDE_DIR=/usr/local/opt/openssl/include/openssl -DCMAKE_CXX_STANDARD=14 -D_FILE_OFFSET_BITS=64 -DCMAKE_CXX_FLAGS=\“-DMIE_ATE_USE_GMP\" -DNDEBUG"
fi
mkdir build
cd build
