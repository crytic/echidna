...
researchTests = testGroup "Research-based Integration Testing"
  ...
  , testContract' "research/harvey_baz.sol" Nothing Nothing Nothing False False
      [ ("echidna_all_states failed", solved "echidna_all_states") ]
  , testContract' "research/ilf_crowdsale.sol" Nothing (Just (\v -> v >= solcV (0,5,0) && v < solcV (0,6,0))) (Just "research/ilf_crowdsale.yaml") False False
      [ ("echidna_assert failed", solved "withdraw") ]
  , testContract' "research/solcfuzz_funwithnumbers.sol" (Just "VerifyFunWithNumbers") (Just (< solcV (0,6,0))) (Just "research/solcfuzz_funwithnumbers.yaml") True False
