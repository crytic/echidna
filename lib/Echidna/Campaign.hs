...
import Control.Monad.State.Strict (MonadState(..), MonadIO, StateT, evalStateT, execStateT, runStateT, liftIO)
...
data TestState = Open Int             -- ^ Maybe solvable, tracking attempts already made
               | Large Int [Tx] Events  -- ^ Solved, maybe shrinable, tracking shrinks tried + best solve
               | Passed               -- ^ Presumed unsolvable
               | Solved [Tx] Events   -- ^ Solved with no need for shrinking
...
instance Semigroup Campaign where
  (Campaign t c g em es) <> (Campaign t' c' g' _ es') = Campaign (t <> t') (c <> c') (g <> g') em (es <> es')

instance Monoid Campaign where
  mempty = Campaign mempty mempty mempty mempty mempty
...
isDone (Campaign ts _ _ _ _) = view (hasLens . to (liftM2 (,) testLimit shrinkLimit)) <&> \(tl, sl) ->
  all (\case Open i -> i >= tl; Large i _ _ -> i >= sl; _ -> True) $ snd <$> ts
...
updateTest :: ( MonadCatch m, MonadRandom m, MonadReader x m, Has TestConf x, Has CampaignConf x)
           => VM -> EventMap -> Maybe (VM, [Tx]) -> (SolTest, TestState) -> m (SolTest, TestState)
updateTest v em (Just (v', xs)) (n, t) = view (hasLens . to testLimit) >>= \tl -> (n,) <$> case t of
  Open i    | i >= tl -> pure Passed
  Open i              -> catch (evalStateT (checkETest n) v' <&> bool (Large (-1) xs $ extractEvents em v') (Open (i + 1)))
                               (pure . Failed)
  _                   -> snd <$> updateTest v em Nothing (n,t)
updateTest v em Nothing (n, t) = view (hasLens . to shrinkLimit) >>= \sl -> (n,) <$> case t of
  Large i x e | i >= sl -> pure $ Solved x e
  Large i x e          -> if length x > 1 || any canShrinkTx x
                           then (do (x', v') <- runStateT (shrinkSeq n x) v
                                    return $ Large (i + 1) x' $ extractEvents em v')
                           else pure $ Solved x e
  _                   -> pure t
...
evalSeq v e = go [] where
  go r xs = do 
               v' <- use hasLens
               c' <- use hasLens  
               runUpdate (updateTest v (c' ^. eventmap) $ Just (v',reverse r)) 
               case xs of []     -> pure ()
                          (y:ys) -> e y >> go (y:r) ys
...
campaign :: ( MonadCatch m, MonadRandom m, MonadReader x m, MonadIO m, Has TestConf x, Has CampaignConf x)
         ...
         -> m Campaign
campaign u v em w ts d = let d' = fromMaybe mempty d in fmap (fromMaybe mempty) (view (hasLens . to knownCoverage))
  >>= \c -> execStateT runCampaign (Campaign ((,Open (-1)) <$> ts) c d' em []) where
    step        = runUpdate (updateTest v em Nothing) >> u >> runCampaign
