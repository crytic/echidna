...
parseAsType t v = case (t, v) of
  ...
  (AbiBoolType, Bool b) -> return (AbiBool b)
...
instance ToJSON AbiValue where
  toJSON = \case
    AbiUInt _ i -> Number (fromIntegral i)
    AbiInt  _ i -> Number (fromIntegral i)
    AbiBool   b -> Bool b
    AbiAddress addr -> Number (fromIntegral addr)
    AbiBytes _      bs -> String (decodeUtf8 bs)
    AbiBytesDynamic bs -> String (decodeUtf8 bs)
    AbiString s -> String (decodeUtf8 s)
    AbiArray _      _ es -> Array (V.map toJSON es)
    AbiArrayDynamic _ es -> Array (V.map toJSON es)
