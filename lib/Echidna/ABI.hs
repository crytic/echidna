...
ppAbiValue (AbiTuple v) =
  "(" ++ intercalate ", " (ppAbiValue <$> toList v) ++ ")"
ppAbiValue (AbiFunction f)       = show f
...
shrinkAbiValue (AbiFunction f  )     = pure $ AbiFunction f
...
mutateAbiValue (AbiFunction f)       = pure $ AbiFunction f
