...
ppTests (Campaign _ _ _ False _ _ _) = pure "starting up, please wait  "
ppTests (Campaign ts _ _ _ _ _ _) = unlines . catMaybes <$> mapM pp ts where
...
ui v w ts d txs = let xfer e = use hasLens >>= \c -> isDone c >>= ($ e c) . bool id forever in do
  ...
  c <- if s then do bc <- liftIO $ newBChan 100
                    ...
                    liftIO (customMain (mkVty defaultConfig) (Just bc) a $ Campaign mempty mempty d' False False txs 0)
